...
int main(){
  ...
  char* bsz = getenv("BATCH_SIZE");//default is 6000
  int ibsz = atoi(bsz);

  // how many dynamic instructions in a loc array
  //char* lsz = getenv("LOC_ARR_SIZE"); 
  //int ilsz = atoi(lsz);

  vector<instr_info> instr_info_set;
  deadloc_detect a_deadloc_detect;
  int tid, nthreads;
  ...
  //string infile="./trace_files/AfraidoftheDark.txt";
  in.open(infile.c_str(), ifstream::in);
  ofstream ou;
  //string outfile="./result.txt";
  string outfile="../NEW_results/"+file+"_deadloc.txt";
  ...
  while(true){
	...
	if(instr_info_set[zs-1].opcodeId==28){
      ...
      //if (n>=ibsz||in.eof()){
      if (n>=ibsz||st1=="az"||st2=="az"||in.eof()){
	//int k;
	// Stopping read of the trace, and starting trace analysis
	//len = instr_info_set.size();
	if(n<ibsz&&st1!="az"&&st2!="az"){
	  ifend=true;
	}else{
	  ifend=false;
	}

	a_deadloc_detect.detector(instr_info_set, ou, st1, st2, ifend);
	#pragma omp parallel for
	for(int i=0;i<zs;i++){
	  instr_info_set[i].oprd_line_set.clear();
	}
...
/*
	for(k=0;k<len;k++){
	  ...
	  // deadlocing pattern detection
	  a_deadloc_detect.detector(instr_info_set, ou, ilsz);
	  ...
	  if (k>=0.8*ibsz){
	    ...
	    printf("the size of trace block after erase is: %d \n", n);
	    instr_info an_instr_info;
	    instr_info_set.push_back(an_instr_info);
	    break;
	  }
